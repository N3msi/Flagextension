class nm_CraftArmbandFlag extends RecipeBase
{	
	override void Init()
	{
	...
	override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight)
	{
		...
		{
			ItemBase armband = results[i];
			armband.SetObjectTexture(2, nmFlagTexture);
			nm_DoubleArmbandDummy armbandDummyResult = nm_DoubleArmbandDummy.Cast(armband);
