class nm_DeCraftSlingbagDummy extends RecipeBase
{	
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"nm_SlingbagDummy");//you can insert multiple ingredients this way
		...
		m_ResultToInventory[0] = false;//(value) == -2 spawn result on the ground;(value) == -1 place anywhere in the players inventory, (value) >= 0 means switch position with ingredient number(value)
	...
	override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight)
	{
		...
		nm_SlingbagDummy slingbag = nm_SlingbagDummy.Cast(ingredients[0]);

		if (slingbag)
		{
			string nmflagName = slingbag.GetnmFlagName();
			...
			if (nmflagName != "")
			{
				...
				if (flag)
				{
					float SlingbagHealth = ingredients[0].GetHealth();

					flag.SetHealth(SlingbagHealth);
				}
			}
		}
	}
