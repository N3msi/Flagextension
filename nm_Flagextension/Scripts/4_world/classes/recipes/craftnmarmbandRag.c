class nm_CraftArmbandRag extends RecipeBase
{	
	override void Init()
	{
	...
	{
		string nmFlagTexture = "nm_Flagextension\\flag\\data\\nm_AB_rag_co.paa";
		
		nm_DoubleArmbandDummy armbandDummyResult = nm_DoubleArmbandDummy.Cast(results[0]);
			if (armbandDummyResult)
			{
				armbandDummyResult.SetObjectTexture(2, nmFlagTexture);
				armbandDummyResult.SetnmFlagTexture(nmFlagTexture);
			}
	}
};
